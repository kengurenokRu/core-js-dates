...
function formatDate(date) {
  ...
  return date2.toLocaleString('en-US', { timeZone: 'UTC' });
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  let dateStop = '';
  if (month === 12) {
    dateStop = new Date(year + 1, 0);
  } else dateStop = new Date(year, month);
  dateStop.setDate(dateStop.getDate() + 1);
  let count = 0;
  while (date < dateStop) {
    if (date.getDay() === 0 || date.getDay() === 1) count += 1;
    date.setDate(date.getDate() + 1);
  }
  return count;
}
...
function getWeekNumberByDate(date) {
  const dateNow = new Date(date.getFullYear(), 0);
  ...
  if (
    ...
    (dateNow.getDay() !== date.getDay() && dateNow.getDay() === 0)
  )
    count += 1;
  return count;
}
...
function getNextFridayThe13th(date) {
  while (true) {
    date.setDate(date.getDate() + 1);
    if (date.getDay() === 5 && date.getDate() === 13) return date;
  }
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const date1 = new Date(period.start.replace(/(\d+).(\d+).(\d+)/, '$3/$2/$1'));
  const date2 = new Date(period.end.replace(/(\d+).(\d+).(\d+)/, '$3/$2/$1'));
  let work = 1;
  let offDay = 1;
  const mas = [];
  while (date1 <= date2) {
